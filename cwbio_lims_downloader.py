...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('cwbio_lims_download.log', mode='a')
    ]
)
...
class FileDownloader:
    ...
    def _perform_download(self, request: DownloadRequest) -> DownloadResult:
        ...
        try:
            ...
            logger.warning(f"HEAD请求失败，无法获取文件大小: {url}, 错误: {str(e)}")
            total_size = 0
...
class CwbioLimsDownloader:
    ...
    def send_api_request(self, args) -> Dict[str, Any]:
        ...
        if not sign_value and not appsecret:
            raise ValueError("配置文件中缺少'sign'或'appsecret'参数")
        
        # 生成签名
        appid_md5 = md5(build_sign(appid, sign_value if sign_value else appsecret))
        ...
        page = args.page if args.page else int(self.config.get('page', '1'))
        limit = args.limit if args.limit else int(self.config.get('limit', '100'))
...
def parse_args():
    ...
    parser.add_argument('--page', type=int, help='页码 (覆盖配置文件)')
    parser.add_argument('--limit', type=int, help='每页记录数 (覆盖配置文件)')
...
配置文件格式:
  ...
  appsecret = your_appsecret                      # 应用密钥
  sign = your_sign                                # 签名（可选，如果提供将不使用appsecret生成）
